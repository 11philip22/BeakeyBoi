...
void main()
{
	...
	if (VirtualProtect(finalShellcode, sysInfo.dwPageSize, PAGE_EXECUTE_READWRITE, &dwOldProtect1)) {
		RDI rdi = reinterpret_cast<RDI>(finalShellcode);

		wprintf(L"[+] Executing RDI\n");
		HMODULE hLoadedDLL = reinterpret_cast<HMODULE>(rdi()); // Excute DLL

		free(finalShellcode); // Free the RDI blob. We no longer need it.

		Function exportedFunction = reinterpret_cast<Function>(GetProcAddressR(hLoadedDLL, "Uninstall"));
		if (exportedFunction) {
			wprintf(L"[+] Calling exported function\n");
			exportedFunction();
		}
	}
